...
def main():
    ...
    # url = f"{config.host}{getattr(config, endpoint)}"
    if use_faker:
        for _ in range(number_of_records):
            response = create_application(generate_product_payload("product", get_id("brand"), "9051"), "product", get_id("advertiser"))
            write_content_into_file(response, "product")
    else:
        data, len = read_content_from_file("product")
        for _ in range(len):
            response=create_application(data[_], "product", get_id("advertiser"))
            write_content_into_file(response, "product")
